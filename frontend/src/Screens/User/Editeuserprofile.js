...
function Editeuserprofile() {
    ...
    useEffect(() => {
      async function getUser() {
        try {
          setLoading(true);
          const response = (
            await axios.post(`http://localhost:5000/api/users/getuser/${u_id}`)
          ).data;
          setLoading(false);
    ...
    async function Updateuser(e) {
      ...
      try {
        ...
        const response = (
          await axios.put(
            `http://localhost:5000/api/users/updateuser/${u_id}`,
            updateuser
          )
        ).data;
        ...
        Swal.fire(
          ...
          window.location.href = `/u_userprofile/${u_id}`;
        });
      } catch (error) {
        setLoading(false);
        console.log(error);
